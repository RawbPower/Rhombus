...
namespace rhombus
{
	...
	void SpriteRendererComponent::UpdateSubTexture() 
	{
		if (m_texture)
		{
			...
			if (m_subtexture)
			{
				m_subtexture->UpdateTexCoords(Vec2(m_frame % m_columns, m_rows - 1 - m_frame / m_columns), spriteSize, (float)m_padding);
			}
			else
			{
				m_subtexture = SubTexture2D::CreateFromCoords(m_texture, Vec2(m_frame % m_columns, m_rows - 1 - m_frame / m_columns), spriteSize, (float)m_padding);
			}
		}
	}
}